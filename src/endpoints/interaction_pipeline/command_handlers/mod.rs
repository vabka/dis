...
use crate::discord::interactions::{Interaction, InteractionCallback, InteractionData, InteractionType};
use crate::endpoints::interaction_pipeline::{InteractionError, InteractionHandler, InteractionHandlerResult, Task};
...
impl<T: CommandHandler<
    Future=Task<CommandHandlerResult>,
    Context=C
>, C> InteractionHandler for T {
    ...
    fn handle(&self, interaction: &Interaction, context: &Self::Context) -> Self::Future {
        ...
        match args {
            Some(Ok(args)) => {
                Box::pin(self.handle(args, context).map(Some))
            }
